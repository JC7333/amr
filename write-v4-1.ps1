# write-v4-1.ps1 — patch v4 -> v4.1
# Fixes : encoding UTF-8 subject/From mail + max_tokens 4500→8000 + Node 24 force
# Aucun changement de comportement, juste des fixes de qualite

Write-Host "=== Switch sur main + pull ===" -ForegroundColor Cyan
git checkout main
git pull
$existsBranch = git branch --list "sync/weekly-pulse-v4-1"
if ($existsBranch) {
    Write-Host "  Branche existe deja, suppression locale" -ForegroundColor Yellow
    git branch -D sync/weekly-pulse-v4-1
}
git checkout -b sync/weekly-pulse-v4-1

Write-Host "=== Ecriture weekly_pulse.py v4.1 ===" -ForegroundColor Cyan
$pyContent = @'
#!/usr/bin/env python3
"""
weekly_pulse.py — Routine 5 AMR (v4.1 — fix encoding UTF-8 + max_tokens).
...
v4.1 fixes (par rapport à v4) :
- Encoding UTF-8 explicite sur Subject/From (fini les "ðŸŸ¢" mojibake)
- max_tokens 4500 → 8000 (fini les pulses tronqués mi-phrase)
- Le reste inchangé : cross-checking + envoi mail + alert level
...
Write-Host "=== Ecriture weekly-pulse.yml (Node 24 force) ===" -ForegroundColor Cyan
$ymlContent = @'
name: weekly-pulse

# Routine 5 d'Audric — revue hebdo automatique dimanche 9h Paris.
# v4.1 : fix encoding UTF-8 + max_tokens + Node 24 forcé.
#
# Secrets requis :
...
#
# Timing :
#   - 0 7 * * 0 = dimanche 7h UTC = 9h Paris en été
#   - 0 8 * * 0 = dimanche 8h UTC = 9h Paris en hiver
#   Le job s'exécute UNE fois selon le mois (cron natif sans DST).
...
# Forcer Node 24 dès maintenant (anticipation deprecation 2 juin 2026)
env:
...
git commit -m "weekly-pulse v4.1 : fix encoding UTF-8 mail + max_tokens 8000 + Node 24"

Write-Host "=== Push + PR + merge ===" -ForegroundColor Cyan
git push origin sync/weekly-pulse-v4-1
gh pr create --title "weekly-pulse v4.1 (fixes)" --body "Fixes : (1) encoding UTF-8 explicite Subject/From mail (fini les mojibake), (2) max_tokens 4500->8000 (fini les pulses tronques mi-phrase), (3) Node 24 force en avance (fini le warning deprecation). Aucun changement fonctionnel."
...
Write-Host ""
Write-Host "Test : Actions > weekly-pulse > Run workflow (attendre 3-4 min)"
Write-Host "Verifier que le mail arrive avec le bon sujet ENCODE (emoji visible, pas mojibake)"
Write-Host "Verifier que le pulse n'est plus tronque a la fin"
