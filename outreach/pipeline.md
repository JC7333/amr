...
- **2026-08-24 (Tracker Reply Tracker)** : Désynchro pipeline ~40j détectée (dernier commit 2026-07-15). **AUCUN changement de statut appliqué automatiquement** — trop de risque de piétiner un état réel non tracé (envois planifiés 16-17/07, 20/07, semaine 20/07 pouvant avoir eu lieu et généré des réponses hors pipeline). Gesnouin remplit mécaniquement la condition >35j silence, mais contexte institutionnel documenté (cf. leçon Sportisse : institutions répondent à 2-3 mois, règle 35j calibrée pour privé) + note explicite "Silence = silence" → **décision manuelle Audric requise**, pas d'auto SLOT LIBÉRÉ. Alertes détaillées dans la PR du run. Action prioritaire : resynchroniser pipeline.md avec l'état réel des 3 envois planifiés + statut EN COURS HUMAIN vieillissants, puis re-déclencher le tracker.
