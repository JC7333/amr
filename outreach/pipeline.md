...
- **2026-08-11 (tracker Claude Code)** : run auto Reply Tracker. ⚠️ Pipeline désynchronisé depuis ~27 jours (dernier commit humain 15/07 v7.1) — toutes les alertes ci-dessous sont potentiellement basées sur des données stale ; envois planifiés Hadj/Boutemadja/MACSF ont pu être exécutés hors Claude Code sans mise à jour du fichier. Aucun changement de statut appliqué automatiquement (aucun ENVOYÉ >35j à purger — Gesnouin à 27j reste dans la fenêtre). Alertes émises dans la PR : 3 rappels PROPOSÉ (Boutemadja, Hadj, MACSF — planifiés depuis le 15/07, non tracés comme envoyés depuis 27j), 3 EN COURS HUMAIN stagnants (Sportisse boucle documentée comme close via cc Gesnouin, Iteanu ~3 mois depuis appel de mai, Hubert ~3,5 mois depuis 29/04). 0 canal alt LinkedIn 14-20j applicable (seul ENVOYÉ actif = Gesnouin en email, hors périmètre canal alt). Verdict action #1 Audric : resynchroniser le pipeline avec la réalité des envois avant tout autre chose.
