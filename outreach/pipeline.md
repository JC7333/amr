...
- **2026-08-06 (Reply Tracker automatique, run du jour)** : ⚠️ Désynchro pipeline 511h (~21j) depuis v7.1. Aucun changement de statut automatique (Gesnouin à J+22 sur 35 — pas encore SLOT LIBÉRÉ ; et pas de fenêtre canal alt car cc Sportisse). Alertes générées (voir PR du run) : 3 rappels PROPOSÉ (Boutemadja/Klaimee, Hadj/Doctolib, MACSF — envois planifiés mi/fin juillet non tracés comme partis), 3 EN COURS HUMAIN stagnants (Sportisse 22j — note "rien à faire de plus" ; Iteanu 93j — attend reconfirmation appel mai ; Hubert 99j — dormant optionnel). Verdict : re-synchroniser le pipeline en priorité avant tout autre action, sinon les prochains runs continueront à dériver.
