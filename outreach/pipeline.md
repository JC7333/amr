...
- **2026-07-26 (v7.1.1)** : Run Reply Tracker (automatique). Aucun changement de statut appliqué (aucun ENVOYÉ n'a atteint 35j — Gesnouin à 11j email, seul actif). Alertes portées dans la PR : 3 EN COURS HUMAIN stagnants >3j (Sportisse 11j / Iteanu ~85j / Hubert ~88j), 3 rappels PROPOSÉ >3j (Hadj / Boutemadja / MACSF — envois recalés semaine 20/07, exécution à confirmer par Audric). Pipeline désynchronisé de ~247h au moment du run (dernier commit 15/07 16:52 UTC) — alertes potentiellement basées sur données stale, resync manuel recommandé avant action.
