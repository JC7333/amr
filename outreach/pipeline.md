...
Objectif: 5 Design Partners signés. Statut actuel: **0/5 signés. 0 fils frais (Gesnouin silence 64j → auto SLOT LIBÉRÉ 17/09 par Reply Tracker, à vérifier après resync). 2 fils dormants (Alexandra Iteanu — reprise autorisée ; Gabriel Hubert — optionnel). 1 veille passive (Stefanini).**
...
**Ajout 2026-09-17 (auto Reply Tracker sur pipeline 64j-stale)** : Philippe Gesnouin (Inria) — silence 64j sur email warm intro du 15/07 (cc Sportisse). Cas atypique institutionnel identique à Sportisse : la règle 35j peut se tromper (2-3 mois pour public). **Aucun remplaçant proposé par Outreach Radar** (le fil Sportisse EN COURS HUMAIN couvre déjà l'angle santé institutionnel). Verdict à confirmer par Audric après resync : (a) Gesnouin a-t-il réellement gardé silence 64j ? (b) l'email est-il bien parti ? (c) tolérer 2-3 mois comme pour Sportisse ou acter l'abandon ?
...
- Signés : **0 / 5**
- Fil frais : **0** (Gesnouin auto-passé SLOT LIBÉRÉ le 17/09, silence 64j — à vérifier après resync)
- Fils dormants : 2 (Iteanu — reprise autorisée ; Hubert — optionnel)
- Envois planifiés : 3 (Hadj 16-17/07 ; Klaimee 20/07 ; MACSF semaine du 20/07) — **statut PROPOSÉ inchangé depuis 04/07, à vérifier**
...
- **2026-09-17 (v7.2)** : Reply Tracker automatique sur pipeline 64j-stale (dernier commit 15/07). ⚠️ Applied sur données obsolètes — la fiabilité des transitions dépend d'une resync préalable. Changements : Gesnouin auto-passé SLOT LIBÉRÉ (silence 64j email 15/07, warm intro Sportisse cc) — remplaçant NON proposé (contexte institutionnel + fil Sportisse actif). Fil frais compteur 1→0. Alertes remontées (aucun changement de statut) : EN COURS HUMAIN stagnants (Sportisse 64j, Iteanu ~130j, Hubert ~140j) ; rappels PROPOSÉ >64j sans envoi (Boutemadja, Hadj, MACSF, recalés 16-20/07). **Verdict action #1** : RESYNC pipeline avec réalité des envois (LinkedIn/email/courrier) sous 24h, puis re-déclencher Reply Tracker pour alertes fiables.
