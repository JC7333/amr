...
- **2026-08-04 (tracker daily)** : Run automatique du Reply Tracker. Aucun changement de statut appliqué (aucun ENVOYÉ >35j : Gesnouin seulement à 20j). Alertes signalées dans la PR : 3 rappels PROPOSÉ (Boutemadja/Klaimee, Hadj/Doctolib, MACSF — tous recalés depuis ~15-19j sans envoi confirmé), 3 alertes EN COURS HUMAIN stagnant (Sportisse 20j — clos par cc Gesnouin, Iteanu ~91j — attente reconfirmation appel, Hubert ~97j — dormant optionnel). Pipeline stale depuis 20 jours (>48h désynchro) — resync manuel prérequis à toute action sur ces alertes.
