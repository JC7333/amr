...
- **2026-08-29 (tracker run, AUCUN changement de statut appliqué)** : Pré-vérification désynchro flag ROUGE : pipeline non modifié depuis 2026-07-15 (~1063h, soit 44 jours). Le tracker n'a rien appliqué automatiquement car les données sont massivement stales — impossible de savoir si Hadj (16-17/07), Boutemadja (20/07), MACSF (semaine 20/07), Iteanu (reprise), ou Gesnouin (réponse) ont bougé hors du pipeline. Rappels : les 3 PROPOSÉ ci-dessus sont dépassés de ~40 jours sur planning. Gesnouin ENVOYÉ depuis 45j : la règle mécanique dit SLOT LIBÉRÉ, mais la note pipeline (leçon Sportisse 15/07) précise que les institutions publiques répondent à 2-3 mois — arbitrage laissé à Audric. **Action prioritaire : resynchroniser pipeline.md avec l'état réel des envois, puis re-déclencher le tracker.** Alertes détaillées dans la PR associée.
