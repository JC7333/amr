...
- **2026-08-23 (v7.2)** : Reply Tracker (Claude Code, run automatique). ⚠️ **Désynchro 38j détectée** (dernier commit pipeline 15/07 → aujourd'hui 23/08). AUCUNE auto-purge appliquée malgré Gesnouin >35j silence : la leçon Sportisse (v7.1) documente explicitement que la règle 35j est calibrée pour le privé, pas les institutions publiques. Alertes surfacées en PR sans modification de statuts. À faire par Audric : **RESYNC pipeline immédiatement** (Zimbra + LinkedIn) avant toute décision, puis re-déclencher le tracker.
