...
- **2026-07-27 (v7.1.2)** : Run Reply Tracker (automatique). Aucun changement de statut appliqué (aucun ENVOYÉ n'a atteint 35j — Gesnouin à 12j email, seul actif). Alertes portées dans la PR : 3 EN COURS HUMAIN stagnants >3j (Sportisse 12j / Iteanu ~85j / Hubert ~89j), 3 rappels PROPOSÉ >3j (Hadj ~28j / Boutemadja 23j / MACSF 23j — envois recalés semaine 20/07, exécution à confirmer par Audric — 3e run consécutif avec les mêmes 3 rappels non tranchés). Pipeline désynchronisé de ~276h (~11j 12h) au moment du run (dernier commit 15/07 16:52 UTC) — alertes potentiellement basées sur données stale, resync manuel recommandé avant action.
